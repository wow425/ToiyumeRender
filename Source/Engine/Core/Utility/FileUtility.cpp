...
namespace Utility
{
    ByteArray NullFile = std::make_shared<std::vector<Utility::byte> >(std::vector<Utility::byte>());
}
...
ByteArray ReadFileHelper(const wstring& fileName)
{
    ...
    Utility::ByteArray byteArray = make_shared<vector<Utility::byte> >(fileStat.st_size);
