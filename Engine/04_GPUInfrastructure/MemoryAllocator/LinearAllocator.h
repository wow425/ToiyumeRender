...
/*
*  CPU端线性分配器，分配虚拟内存的内存池，只负责划拨内存，不负责管理已分配对象
*  按页批发内存： 4K（操作系统默认内存分页大小）,    2MB（现代CPU支持的大页内存）
*  2MB的加速页表漫游PTB（Page Table Walk），尽量减少Cache Miss
*/


#include "03_RHI/Resource/GpuResource.h"
#include <vector>
#include <queue>
#include <mutex>
