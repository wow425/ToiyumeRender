...
namespace Renderer
{
	Renderer::RendererAutoRegister<Renderer::testRenderer> s_RegisterforwardRenderer(L"testRenderer");
}


namespace Renderer
{
	bool testRenderer::Initialize(const RendererCreateDesc& desc)
	{
	...
	void testRenderer::OnResize(uint32_t width, uint32_t height)
	{
	...
	void testRenderer::BeginFrame(const RenderFrameDesc& frame)
	{
		(void)frame;
	}

	void testRenderer::Update(const RenderFrameDesc& frame)
	{
		(void)frame;
	...
	void testRenderer::Render(MeshSorter::DrawPass pass, GraphicsContext& context, GlobalConstants& globals, const RenderFrameDesc& frame)
	{
	...
	void testRenderer::EndFrame(::GraphicsContext& frameContext, const RenderFrameDesc& frame)
	{
		(void)frameContext;
		(void)frame;
	}

	void testRenderer::BuildDescriptorHeaps()
	{
	...
	void testRenderer::BuildRootSignature()
	{
		using namespace Renderer;
		...
		m_RootSig.Finalize(L"testRenderer RootSig", D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
	}

	void testRenderer::BuildPSOs()
	{
		// “基础 PSO 模板”
		...
		GraphicsPSO defaultPSO(L"testRenderer: Default PSO");
	...
	void testRenderer::CreateForwardBufferTargets()
	{
		...
		m_ForwardBuffer.SceneColorBuffer.Create(L"Scene Color Buffer", width, height, 1, m_CreateDesc.backBufferFormat);
		m_ForwardBuffer.DepthBuffer.Create(L"Scene Depth Buffer", width, height, m_CreateDesc.depthBufferFormat);


	}

	void testRenderer::DestroyForwardBufferTargets()
	{
		m_ForwardBuffer.DepthBuffer.Destroy();
		m_ForwardBuffer.SceneColorBuffer.Destroy();
	}

	void testRenderer::UpdateGlobalDescriptors()
	{
		// 这里以后放：
		// - 阴影图 SRV
		// - SSAO 结果 SRV
	...
	uint8_t testRenderer::GetPSO(uint16_t psoFlags)
	{
