...
// MeshSorter::AddMesh 根据描述符在运行时动态映射 PSO
	...
	D3D12_GPU_VIRTUAL_ADDRESS bufferPtr)
{
	...
	// 【渲染策略层核心】：在这里动态拼装 PipelineDesc
	PipelineDesc desc;
	desc.VertexFlags = mesh.vertexFlags;
	desc.MaterialFlags = material.Flags;

	// 如果当前处于 Shadow Pass 阶段，直接覆盖 PassType，获取无 Pixel Shader 的深度图 PSO
	desc.PassType = (m_BatchType == kShadows) ? RenderPassType::Shadow : RenderPassType::Forward;

	// 向全局管线管理器请求 PSO 索引（若不存在则立即动态编译或触发后台编译）
	// uint16_t psoIdx = PipelineStateManager::GetPSO(desc); 
	uint16_t psoIdx = 0; // 伪代码，接入你自己的 PipelineStateManager
	...
	{
		if (material.IsAlphaBlend()) // 剔除半透明阴影
			return;
