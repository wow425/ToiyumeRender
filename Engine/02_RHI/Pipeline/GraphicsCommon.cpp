...
#include "02_RHI/Command/CommandSignature.h"
...
namespace Graphics
{
	...
	D3D12_RASTERIZER_DESC RasterizerFullScreen;    // 禁用 Cull
...
void Graphics::InitializeCommonState(void)
{
	...
	RasterizerDefault.FrontCounterClockwise = TRUE;         // 采用右手坐标系，逆时针为前面
	...
	// --- 3. 全屏三角形光栅化状态 ---
	RasterizerFullScreen.FillMode = D3D12_FILL_MODE_SOLID;
	RasterizerFullScreen.CullMode = D3D12_CULL_MODE_NONE;      // D3D12_CULL_MODE_NONE  D3D12_CULL_MODE_BACK
	RasterizerFullScreen.FrontCounterClockwise = TRUE;         // 采用右手坐标系，逆时针为前面
	RasterizerFullScreen.DepthBias = D3D12_DEFAULT_DEPTH_BIAS;
	RasterizerFullScreen.DepthBiasClamp = D3D12_DEFAULT_DEPTH_BIAS_CLAMP;
	RasterizerFullScreen.SlopeScaledDepthBias = D3D12_DEFAULT_SLOPE_SCALED_DEPTH_BIAS;
	RasterizerFullScreen.DepthClipEnable = TRUE;
	RasterizerFullScreen.MultisampleEnable = FALSE;
	RasterizerFullScreen.AntialiasedLineEnable = FALSE;
	RasterizerFullScreen.ForcedSampleCount = 0;
	RasterizerFullScreen.ConservativeRaster = D3D12_CONSERVATIVE_RASTERIZATION_MODE_OFF;
	...
	DepthStateReadOnly = DepthStateDisabled; // 可用于skybox
	DepthStateReadOnly.DepthEnable = TRUE;
	DepthStateReadOnly.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;
	DepthStateReadOnly.DepthFunc = D3D12_COMPARISON_FUNC_LESS_EQUAL;
